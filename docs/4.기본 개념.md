...
### Lifecycle 메서드
Class 컴포넌트에는 컴포넌트의 생명주기 동안 특정 시점에 호출되는 여러 메서드가 있습니다. 주요 메서드는 다음과 같습니다.

- `componentDidMount()`: 컴포넌트가 마운트된 직후에 호출됩니다. 여기서 API 호출이나 구독 설정을 할 수 있습니다.
- `componentDidUpdate(prevProps, prevState)`: 컴포넌트가 업데이트된 직후에 호출됩니다. 여기서 DOM 조작이나 네트워크 요청을 할 수 있습니다.
